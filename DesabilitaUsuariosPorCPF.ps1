# Script para BLOQUEAR usuarios em massa com base em CPF
# A lista de usuarios a bloquear deve ser fornecida via arquivo .csv
# O arquivo .csv deve conter uma coluna intitulada CPF (nome CPF no cabeçalho)
...
$cpfs = New-Object System.Collections.Generic.List[System.Object]
...
$cpfstrings = $cpfs | ForEach-Object {"$($_)"}
...
$usersToBlock = (Get-ADUser -SearchBase $domain -Filter * -Properties mail, sAMAccountName | Where-Object {$_.mail -in $cpfs -or $_.mail -in $cpfstrings})
